...
2. 选择唯一的动词开头的 `change-id`，并在 `openspec/changes/<id>/` 下搭建 `proposal.md`、`tasks.md`、可选的 `design.md`，以及规范增量。
...
- ADDED：引入可以作为独立需求存在的新功能或子功能。当更改是正交的（例如，添加"斜杠命令配置"）而不是改变现有需求的语义时，优先使用 ADDED。
...
4) 确保标题文本完全匹配（不区分空格）并至少保留一个 `#### Scenario:`。
